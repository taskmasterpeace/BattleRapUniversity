      ...
      <div className="p-6 space-y-6">
          ...
          <div className="border-t border-zinc-700 pt-3 space-y-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-200">{playerPerformance.bestMoment}</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-sm text-zinc-200">{playerPerformance.worstMoment}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="space-y-1">
              {playerPerformance.effectiveContent.map((content) => (
                <motion.div
                  key={content}
          ...
          <div>
            ...
            <div className="space-y-1">
              {playerPerformance.ineffectiveContent.map((content) => (
                <motion.div
                  key={content}
                  initial={{ opacity: 0, x: 10 }}
                  animate={{ opacity: 1, x: 0 }}
        ...
        <div className="border-t border-zinc-700 pt-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs text-zinc-400">Low: {playerPerformance.crowdLowPoint}</span>
            <div className="flex-1 h-3 bg-zinc-800 rounded overflow-hidden relative">
              <motion.div
                initial={{ width: 0 }}
                animate={{ width: `${playerPerformance.crowdHighPoint}%` }}
              ...
              <div
                ...
                style={{ left: `${playerPerformance.crowdLowPoint}%` }}
              />
              {/* High point marker */}
              <div
                className="absolute top-0 bottom-0 w-0.5 bg-green-400"
                style={{ left: `${playerPerformance.crowdHighPoint}%` }}
              />
            </div>
            <span className="text-xs text-zinc-400">High: {playerPerformance.crowdHighPoint}</span>
          </div>
        </div>
      </div>
    </div>
